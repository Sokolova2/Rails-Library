...
RSpec.describe SetRandomAvatarService, type: :service do
  ...
  describe '#set_random_avatar' do
    context 'when user have gender' do
      ...
      let(:female_files) { Rails.root.glob('app/assets/images/female/*').map { |f| File.basename(f) } }

      before { service.set_random_avatar }

      it 'sets gender-specific avatar' do
        expect(user_with_gender.avatar).to be_present
      end

      it 'sets avatar from the correct directory' do
    ...
    context 'when user do not have gender' do
      ...
      let(:animal_files) { Rails.root.glob('app/assets/images/animal/*').map { |f| File.basename(f) } }

      before { service.set_random_avatar }

      it 'sets not gender-specific avatar' do
        expect(user_without_gender.avatar).to be_present
      end

      it 'sets avatar from animal directory' do
